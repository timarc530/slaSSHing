...
class myPrompt(wx.Dialog):
    ...
    def __init__(self, parent, title):
        ...
        fgs_name_and_host = wx.FlexGridSizer(rows=8, cols=2, vgap=9, hgap=25)
        fgs_key_file = wx.FlexGridSizer(rows=4, cols=2, vgap=5, hgap=10)
        ...
        fgs_name_and_host.AddMany([(name_label, 1, wx.EXPAND), (self.name_string, 1, wx.EXPAND),
                                   (host_label, 1, wx.EXPAND), (self.host_string, 1, wx.EXPAND),
                                   (password_label, 1, wx.EXPAND), (self.password_string, 1, wx.EXPAND)])
        fgs_key_file.AddMany([self.key_button, (self.key_label, 1, wx.EXPAND),
                             (0, 0), (self.rb_RSA, 1, wx.EXPAND),
                             (0, 0), (self.rb_DSA, 1, wx.EXPAND),
                             (0, 0), (self.rb_ECDSA, 1, wx.EXPAND)])
        fgs_control_buttons.AddMany([self.submit_button, self.cancel_button])

        fgs_name_and_host.AddGrowableCol(1)

        hbox.Add(fgs_name_and_host, proportion=1, flag=wx.ALL | wx.EXPAND, border=15)
        hbox.Add(fgs_key_file, proportion=1, flag=wx.ALL | wx.EXPAND, border=10)
